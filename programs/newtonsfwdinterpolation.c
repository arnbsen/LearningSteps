...
int main(){
    float x[20],dif_table[10][20],res,sum,prod,xn;
    int i,j,n,h;
    ...
    for (i=1; i<n; i++) {//Column Selector
        for (j=1; j<n-i+1; j++) { //Row Selector
            dif_table[j-1][i] = dif_table[j][i-1] - dif_table[j-1][i-1];
        }
    }
    //Evaluating the final Results
    res = 0;
    for (i=0; i<n; i++) {
        prod = dif_table[0][i]/(powf(h,i)*factorial(i));
        for (j=0; j<i; j++) {
            prod = prod*(xn - x[j]);
        }
        res = res + prod;
